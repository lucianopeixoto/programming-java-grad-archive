...
public class ClasseJogo extends GameCanvas implements Runnable{
	...
	// Definindo as sequencias de animação do player
	...
	public ClasseJogo (){
		...
		// Inicializando coisas importantes:
		sorteio = new Random();
		graphics = getGraphics();
		layerManager = new LayerManager();

		// Inicializando os objetos de camada:
		...
		spritePlayer.defineCollisionRectangle(25, 40, 14, 24);
		...
		// preenchimento dos backgrounds finalizado

		// Setando o jogo para o modo Fullscreen
		this.setFullScreenMode(true);

		// Setando o view window do layer manager para não ocupar toda a tela
		...
		// Iniciando um novo jogo e selecionando o estado para mostrar a partida rodando
		novoJogo();
		estadoJogo = jogoRodando;
	}

	void novoJogo(){
		// Inicializando o que for preciso para um jogo acontecer do começo:
	...
	public void run() {
		while (thread == Thread.currentThread()) {
			...
			while (estadoJogo == jogoRodando) {
				...
				// Checa se o player foi levado até o teto e toma suas providencias
				if (yPlayer < -64) {
					vidas--;
					xPlayer = 230/2 - spritePlayer.getWidth()/2;
					yPlayer = 200;
					sorteiaBuraco();
					tiledChao.setPosition(0, 500);
					try {
						Thread.sleep(1000);
					}
					catch( InterruptedException e ){
				...
				// Setando a velocidade de acordo com a pontuação:
				...
				if (pontos > 400000) velocidade = 17;
				if (pontos > 500000) velocidade = 18;
				...
				// Contador para atrasar a animação do jogador
				contaPlayer++;
				if (contaPlayer == 3){
				...
				contaBackground++;
				if (contaBackground > (18-velocidade)/2){
